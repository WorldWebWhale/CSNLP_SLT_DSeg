...
from utils.cal import vt_align_step
...
def build_components():
    ...
    dataset = PhoenixSpaMoLazy(
        ...
        cache_dir="/iopsstor/scratch/cscs/ccotrini/cache_spamo/cache_spamo",
    ...
    logit_temp = torch.nn.Parameter(torch.tensor(math.log(1/0.07)))  
    trainable.append(logit_temp)
    ...
    return sign2t5, llm, tokenizer, adapter, optimizer, scheduler, dataset, loader,trainable, logit_temp
...
sign2t5, llm, tokenizer, adapter, optimizer, scheduler, dataset, loader, trainable, logit_temp = build_components()

sign2t5, llm, tokenizer, adapter, optimizer, loader, scheduler, logit_temp = accelerator.prepare(
    sign2t5, llm, tokenizer, adapter, optimizer, loader, scheduler, logit_temp
)
...
run_name = f"spamo_sadfusionloss2_vtalign_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
for epoch in range(EPOCHS):
    ...
    for step, batch in enumerate(loader):
        with accelerator.accumulate(sign2t5):
            ...
            cl_loss = vt_align_step(fused, batch["tgt"], tokenizer=tokenizer, llm=llm, temperature=logit_temp)
            ...
            loss = (alpha * output.loss + cl_loss) / GRAD_ACCUM
    ...
    ckpt_dir = f"checkpoints/spamo_sadfusion_vtalign_epoch_{epoch}_{timestamp}"
