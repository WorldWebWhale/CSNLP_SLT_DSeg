...
from utils.dynamic_chunker import DynamicChunker, pad_chunk_batch
...
def build_components():
    ...
    chunker = DynamicChunker(dim=D_MODEL, max_chunk_len=32)
...
run_name = f"spamo_sadfusion_lmk_dseg_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
for epoch in range(EPOCHS):
    ...
    ckpt_dir = f"checkpoints/spamo_sadfusion_dynamic_lmk_epoch_{epoch}_{timestamp}"
