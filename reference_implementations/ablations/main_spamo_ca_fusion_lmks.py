...
from utils.fusion_modules import SinusoidalPE, ChunkSpanEmbedding, FusionBlock, CrossAttentionFusion
...
def build_components():
    ...
    fusion = CrossAttentionFusion(
        spatial_dim   = D_SPATIAL,    # e.g. 2048 from CLIP
        flow_dim      = D_MOTION,     # 1024 from VideoMAE CLS
        landmark_dim  = D_LMK,         # landmarks in this ablation
        d_model       = D_MODEL,      # 2048 (T5 hidden)
        use_span      = False,        # disable span encoding
        nhead         = 8,
        num_blocks    = 2,
    )
    ...
    trainable     = lora_params + fusion_params
    ...
    return sign2t5, llm, fusion, optimizer, scheduler, dataset, loader,trainable
...
sign2t5, llm, fusion, optimizer, scheduler, dataset, loader, trainable = build_components()

sign2t5, llm, fusion, optimizer, loader, scheduler = accelerator.prepare(
    sign2t5, llm, fusion, optimizer, loader, scheduler
)
...
run_name = f"spamo_cross_attention_fusion_lmk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
for epoch in range(EPOCHS):
    ...
    ckpt_dir = f"checkpoints/spamo_cafusion_lmk_epoch_{epoch}_{timestamp}"
...
for batch in test_loader:
    ...
    fused = fusion(
        spatial_feats   = spatial,   # (B, T, D_spa)
        flow_feats      = motion,    # (B, N, D_m)
        spans           = None,      # no span encoding
        landmark_feats  = lmk       # no landmarks
    )
...
sign2t5.eval(); fusion.eval(); llm.eval()
torch.set_grad_enabled(False)
