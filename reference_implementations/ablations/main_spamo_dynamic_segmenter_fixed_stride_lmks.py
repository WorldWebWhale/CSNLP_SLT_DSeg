...
from utils.helper import spatial_encoder, motion_encoder, sample_with_stride, PhoenixSpaMoLazy, collate_fn
from utils.dynamic_chunker import FixedStrideMeanChunker, pad_chunk_batch

# IN CASE YOU ALREADY STARTED TRAINING, AND WANT TO START BACK FROM WHERE YOU STOPPED, FILL THIS VARIABLE
# Example : ckpt_dir   = "checkpoints/spamo_sadfusion_epoch_15_20250531_173016" 
ckpt_dir   = ""   
...
def build_components():
    ...
    adapter = SignAdapter(D_SPATIAL, D_MOTION, d_lmk=D_LMK, d_model=D_MODEL) 
    
    chunker = FixedStrideMeanChunker(dim=D_MODEL)
    ...
    train_csv = "phoenix_dir/PHOENIX-2014-T/annotations/manual/PHOENIX-2014-T.train.corpus.csv"
    png_root  = "phoenix_dir/PHOENIX-2014-T/features/fullFrame-210x260px/train"
    
    # Creates a Phoenix Dataset based on what was done previously
    dataset = PhoenixSpaMoLazy(
        root="./phoenix_dir/PHOENIX-2014-T",
        split="train",
    ...
    fusion_params = list(adapter.parameters())
    chunker_params = list(chunker.parameters())
    trainable     = lora_params + fusion_params + chunker_params
    ...
    return sign2t5, llm, adapter, chunker, optimizer, scheduler, dataset, loader,trainable
...
accelerator = Accelerator(mixed_precision="fp8", kwargs_handlers=[ddp_kwargs])
device = accelerator.device

sign2t5, llm, adapter, chunker, optimizer, scheduler, dataset, loader, trainable = build_components()

sign2t5, llm, adapter, chunker, optimizer, loader, scheduler = accelerator.prepare(
    sign2t5, llm, adapter, chunker, optimizer, loader, scheduler
)
...
run_name = f"spamo_sadfusion_lmk_fixedseg_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
for epoch in range(EPOCHS):
    ...
    for step, batch in enumerate(loader):
        with accelerator.accumulate(sign2t5):
            ...
            lmk      = batch["landmarks"].to(device)    # (B, L, 129)

            fused = adapter(spatial, motion, lmk)

            chunk_list = chunker(fused) 
            chunks, chunk_mask  = pad_chunk_batch(chunk_list)         # (B, N_max, D)

            output = sign2t5(
                sign_feats      = chunks,
                sign_mask      = chunk_mask,
    ...
    ckpt_dir = f"checkpoints/spamo_sadfusion_fixedseg_lmk_epoch_{epoch}_{timestamp}"
...
for batch in test_loader:
    ...
    gt      = batch["tgt"][0]               # gold German sentence

    motion   = batch["motion"].to(device)   # (B, N, D_m)
    spatial  = batch["spatial"].to(device)  # (B, T, D_s)
    lmk      = batch["landmarks"].to(device)    # (B, L, 129)
...
sign2t5.eval(); adapter.eval(); chunker.eval(); llm.eval()
torch.set_grad_enabled(False)
